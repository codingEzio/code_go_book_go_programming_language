...
// The usage of the API has already changed, and requiring signup with an email,
// most importantly, it filtered out those temp-email providers (F word to the dev)
...
func (movie Movie) posterFilename() string {
	ext := filepath.Ext(movie.Poster)
	title := slugify.Marshal(movie.Title)
	return fmt.Sprintf("%s_(%s)%s", title, movie.Year, ext)
}

func (movie Movie) writePoster() error {
	url_ := movie.Poster
	resp, err := http.Get(url_)
	if err != nil {
		return err
	...
	file, err := os.Create(movie.posterFilename())
	if err != nil {
		return err
	}
	defer file.Close()
