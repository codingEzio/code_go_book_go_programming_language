// This one is NOT written by myself.
// The original link: https://github.com/torbiak/gopl/blob/master/ex4.12/main.go
package main
...
const usage = `./PROGRAM get 523
./PROGRAM index OUTPUT_FILE		(doesn't work right now)
./PROGRAM search INDEX_FILE QUERY	(doesn't work either)`
...
func main() {
	...
	case "get":
		...
		n, err := strconv.Atoi(os.Args[2]) // ASCII -> Integer
		if err != nil {
...
func index(filename string) error {
	...
	err = gobEncoder.Encode(wordIndex)
	if err != nil {
		return err
	}

	err = gobEncoder.Encode(numIndex)
	if err != nil {
		return err
	}
...
func readIndex(filename string) (WordIndex, NumIndex, error) {
	...
	err = gobDecoder.Decode(&wordIndex)
	if err != nil {
		return nil, nil, err
	}

	gobDecoder.Decode(&numIndex)
	if err != nil {
		return nil, nil, err
	}
